...
# using Clarabel # Doesn't support Binary variables
...
# ╔═╡ 362c114c-d207-4819-a721-c760f0070fba
md"""
If the only nonlinearities in your model are quadratic terms (that is, multiplication between two decision variables), you can also use second-order cone solvers.
...
begin
	...
	tau_min, tau_max = -16, 16
	
	@show tau_values = [2.0^i for i in tau_min:1:tau_max]
	# @variable(model, 1<=tau_index<=length(tau_values), Bin)
	@variable(model, tau_active[1:length(tau_values)], Bin)
	...
	phi = (1 + sqrt(5))/2
	# @constraint(model, phi-v*tau_values[tau_index] <= delta)
	# @constraint(model, v*tau_values[tau_index]-phi <= delta)
