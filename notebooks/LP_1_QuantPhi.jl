...
# ╔═╡ cb55e7a0-85fa-11ef-2c6b-9fcd30ff4a00
using JuMP, HiGHS, Ipopt, Juniper, Alpine, EAGO, DAQP

# ╔═╡ 10353e71-1a9f-4175-8581-59e01e2d6ff2
import MultiObjectiveAlgorithms as MOA

# ╔═╡ cde6ea6f-0e5d-477c-b161-f728e58b0ace
begin
	# model = Model(HiGHS.Optimizer) # doesn't support float
...
# ╔═╡ 37e8cf4a-815a-44c0-a5f9-ee11590d9ecb
phi = (1 + sqrt(5))/2

# ╔═╡ cb517bc9-93bb-426a-9e60-1b1838514ec6
md"""
# Optimal Number System for Quantised Phi Value

$\phi=\frac{1+\sqrt{5}}{2} = 1.618...$

Optimise a number system, defined by the set of parameters below, for the accuracy in its representation of $\phi$.

- $\beta=2$, the radix (constant for now)
- $\tau$, an exponential offset
- $|W|$, the word-length
- $|E|$, exponential field length
- $|M|$, mantissa field length

With the decoded value given below:
$\mathcal{D}_{\Re_{\ge 0}}(\chi) = \beta^{e-\tau}\times m$

"""

# ╔═╡ 6f817355-94b0-455d-bcb6-c1b94883e17e
W = 4

# ╔═╡ 528407b8-ce16-4b9b-8b3a-44374e7eb1d8
tau = -(W-1)

# ╔═╡ 384ba4ba-cd7e-4ca9-88f3-f61d99bdfe58
tau_factor = 2.0^tau

# ╔═╡ 8d6b7a82-5e00-4945-894d-f2bb2e0d8a4f
b = @variable(model, [1:W], Bin)

# ╔═╡ 41c32e0a-ee5d-457a-bce2-2d4074fa98bd
comp = @variable(model, [1:W])

# ╔═╡ fbd22ab9-26be-455f-8799-c855e5e9bf24
@constraint(model, [i = 1:W], comp[i]==1<<(i-1))

# ╔═╡ 7459140d-9c53-4e36-8be1-9b501ce5edcf
qphi = @variable(model, qphi)

# ╔═╡ fb304bad-b288-48a0-be06-a3492ee5bfc6
@constraint(model, [i in 1:W], qphi == sum(b[i]*comp[i] for i in 1:W)*tau_factor)

# ╔═╡ dae29e93-9cc7-4bfe-87f0-851ffc22f032
#@objective(model, Min, sum(b[i]*comp[i] for i in 1:W)-phi)
@objective(model, Min, qphi-phi)
# @objective(model, Min, [qphi-phi, phi-qphi])

# ╔═╡ a1aba900-9a08-440b-8d98-8a6c74d2efba
model

# ╔═╡ 9264d21c-9faf-44ca-b0b7-46cab8e570cb
optimize!(model)

# ╔═╡ 777057dd-2565-46da-a08b-6bb68b7fa604
value.(b)

# ╔═╡ ea779b3a-4c48-4889-aee1-112202679a0f
value.(comp)

# ╔═╡ 21d56e7a-6950-4af2-9862-055459dca91a
value(qphi)

# ╔═╡ 81535e4d-1e00-459a-89ee-5de226e1e098
sum(bi*value(comp[i]) for (i, bi) in enumerate(value.(b)))
...
manifest_format = "2.0"
project_hash = "5bdb03b8b3b3b4d6a6fb0c952c03381a39e4a19f"
...
git-tree-sha1 = "966533235235223672f87dfac0793940cae53a23"
...
[[deps.DAQP]]
deps = ["DAQP_jll", "LinearAlgebra", "MathOptInterface"]
git-tree-sha1 = "c73f650ee3cfe55ca843a65eaab1629e47a7862b"
uuid = "c47d62df-3981-49c8-9651-128b1cd08617"
version = "0.5.2"

[[deps.DAQP_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl"]
git-tree-sha1 = "6e25dea369e8cb8a36c9b9a103408b7ba296f4a7"
uuid = "5c51c916-43bf-57fe-9d62-13064ebbf40d"
version = "0.5.0+0"
...
[[deps.EAGO]]
deps = ["Cassette", "Cbc", "DataStructures", "DocStringExtensions", "FastRounding", "ForwardDiff", "IntervalArithmetic", "IntervalContractors", "Ipopt", "JuMP", "LinearAlgebra", "MINLPTests", "MathOptInterface", "McCormick", "NaNMath", "PrettyTables", "Printf", "Reexport", "Requires", "SparseArrays", "SpecialFunctions"]
git-tree-sha1 = "0e0e93b1302f6adabf6cb9e2218d33eecfb71a50"
...
[[deps.MINLPTests]]
deps = ["JuMP", "Test"]
git-tree-sha1 = "dac56e7d8e912d975bc6231a557489beba90462d"
...
[[deps.McCormick]]
deps = ["DiffRules", "DocStringExtensions", "ForwardDiff", "IntervalArithmetic", "IntervalRootFinding", "LinearAlgebra", "NNlib", "NaNMath", "Reexport", "SpecialFunctions", "StaticArrays", "UnPack"]
git-tree-sha1 = "919bddba6ef9fc141f4ad2b97715ac9577c8be10"
...
git-tree-sha1 = "008b44e2b491b0af88a9d2fa3e625cb4d77a85b7"
