...
# ╔═╡ 1389972c-92e4-11ef-2ee6-e1aab00c0724
using JuMP

# ╔═╡ e3701e09-3763-4450-91ba-7ff993edb801
begin
	ENV["GRB_LICENSE_FILE"] = "/work/gurobi.lic"
	using Gurobi
end

# ╔═╡ 32a8a1ba-fccc-44f9-9f12-ce11337b292d
begin
	model = Model(Gurobi.Optimizer)
	set_attribute(model, "TimeLimit", 100)
	set_attribute(model, "Presolve", 0)

	options = [4, 5, 3, 8, 4, 5, 8]
	option_count = length(options)
	@variable(model, sel[1:option_count], Bin)
	@constraint(model, sum(sel[i] for i in 1:option_count) == 1)
	@variable(model, largest)
	@constraint(model, sum(sel[i]*options[i] for i in 1:option_count) == largest)

	@constraint(model, [i = 1:option_count], largest >= options[i])
