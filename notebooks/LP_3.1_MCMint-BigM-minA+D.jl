...
# ╔═╡ b36e139d-a943-4196-905d-552856136f56
using Printf

# ╔═╡ 8fdcf8a9-2f4f-4c45-933e-d74bcf3a5653
using Serialization

# ╔═╡ 8499c9bc-b987-4653-818e-c3f97eb243e5
include("LP_mcm.jl")
...
begin
	...
	set_attribute(model, "IntegralityFocus", 1)
	set_attribute(model, "MIPFocus", 3)
	set_attribute(model, "ConcurrentMIP", 2)
	...
	input_shift_values = [2^e for e in 0:9]
	...
	@constraint(model, [v = 1:nof_output_values, a = 1:nof_adders],
		output_values[v] - (1 - adder_output_value_sel[v, a])*delta_M <= adder_outputs[a]
	)
	@constraint(model, [v = 1:nof_output_values, a = 1:nof_adders],
		adder_outputs[a] <= output_values[v] + (1 - adder_output_value_sel[v, a])*delta_M
	)
	...
	@constraint(model,
		sum(adder_depth_max_sel[a] for a in 1:nof_adders) == 1
	)
	@constraint(model, [a = 1:nof_adders],
...
# ╔═╡ 9a286a6f-1707-4867-b529-552ec16c0092
result_count(model)

# ╔═╡ e08c7b49-649f-4a13-808c-b95a015d6c86
...
value.(adder_input_shifted; result=3)
...
# ╔═╡ 664254a2-9e7b-449a-9ba2-08b872fc6fdd
value(adder_depth_max)

# ╔═╡ 28bce9a7-92d5-4024-9047-4d1d1fc93e0c
all_ao = [value.(adder_outputs;result=i) for i in 1:result_count(model)]

# ╔═╡ 6ab61e39-4639-470d-97f1-713f22af6af5
un_ao = unique(all_ao)

# ╔═╡ 607301e7-9638-4daa-b336-fcc3b87c1414
[findfirst(x->x==un, all_ao) for un in un_ao]
...
manifest_format = "2.0"
project_hash = "c1c33c6d036479c6f427c48b2d43e15fcb8f197b"
...
# ╠═607301e7-9638-4daa-b336-fcc3b87c1414
# ╠═8499c9bc-b987-4653-818e-c3f97eb243e5
# ╠═a00aff9a-cbfb-4def-8c6f-e6f8f83320ce
# ╠═8e06f2c1-8a5c-484e-aaed-3c4f34a39c42
# ╠═b36e139d-a943-4196-905d-552856136f56
# ╠═8fdcf8a9-2f4f-4c45-933e-d74bcf3a5653
# ╠═dd151c05-57bf-46dd-bc5a-972569651d56
# ╠═e39a4b43-1caf-4653-aa77-51be76db5c07
# ╠═7f25e752-49da-47ca-8214-1c3bb4ebc0ef
# ╠═1096b34d-d082-4446-aea0-07b07744fb50
# ╠═2dd7a056-29b0-40a4-af1d-74e5f3abb623
# ╠═ff68f4c4-8bc4-4fb4-b655-6c478a120152
# ╠═9a7e7c02-9b5d-47d0-8f4f-05f4c2995b2e
# ╠═ef26d480-d128-4142-b6d7-eeb363bac215
# ╠═62365a6a-29d3-445a-ab1a-c0dbc37e907d
# ╠═5b5e8537-00e1-4ceb-a4d3-d5576fb373ec
# ╟─00000000-0000-0000-0000-000000000001
# ╟─00000000-0000-0000-0000-000000000002
