...
# ╔═╡ be1ab380-9517-11ef-34e3-cb30a2c80ae2
using JuMP

# ╔═╡ 201552a1-87e5-4f30-8935-b7a4c0f4243c
using HiGHS

# ╔═╡ b38e3070-7463-4387-b3ef-439bbbc3f98a
begin
	ENV["GRB_LICENSE_FILE"] = "/work/gurobi.lic"
	using Gurobi
end

# ╔═╡ f82b24f0-4213-401d-99a1-103e6ad7a99a
begin
	model = Model(Gurobi.Optimizer)
	set_attribute(model, "TimeLimit", 600)
	set_attribute(model, "Presolve", 1)
	set_attribute(model, "IntegralityFocus", 0)
	set_attribute(model, "MIPFocus", 1)
	# set_attribute(model, "ConcurrentMIP", 6)
	...
	# nof_adders, output_values = 7, [−8, 4, 28, 5, −67, −44, 175]
	nof_adders, output_values = 7, [287, 307, 487]
	# nof_adders, output_values = 4, [307, 439]
	nof_output_values = length(output_values)
	
	input_sign_values = [-1, 1]
	nof_input_sign_values = length(input_sign_values)
	
	input_shift_values = [2^e for e in 0:8]
	nof_input_shift_values = length(input_shift_values)

	# Adders have outputs.
	@variable(model,
		adder_outputs[0:nof_adders],
	Int)
	...
	@constraint(model, [a = 1:nof_adders],
		sum(adder_output_value_sel[v, a] for v in 1:nof_output_values) <= 1
	)
	@constraint(model, [v = 1:nof_output_values],
		sum(adder_output_value_sel[v, a] for a in 1:nof_adders) == 1
	)
	## Constrain adder output to its selected value by indicating on difference
	@variable(model,
		adder_output_value_delta[
			1:nof_output_values,
			1:nof_adders
		],
	Int
	)
	@constraint(model, [v = 1:nof_output_values, a = 1:nof_adders],
		adder_output_value_delta[v, a] == adder_outputs[a]-output_values[v]
	)
	delta_M = 2*maximum_value
	@constraint(model, [v = 1:nof_output_values, a = 1:nof_adders],
		0 - delta_M + adder_output_value_sel[v, a]*delta_M <= adder_output_value_delta[v, a]
	)
	@constraint(model, [v = 1:nof_output_values, a = 1:nof_adders],
		adder_output_value_delta[v, a] <= 0 + delta_M - adder_output_value_sel[v, a]*delta_M
	)
	...
	@constraint(model, [s = 1:nof_input_shift_values, i = 1:nof_adder_inputs, a =1:nof_adders],
		adder_input_value[i,a]*input_shift_values[s] - shifted_M + adder_input_shift_sel[s, i, a]*shifted_M <= adder_input_shifted[i, a]
	)
	@constraint(model, [s = 1:nof_input_shift_values, i = 1:nof_adder_inputs, a =1:nof_adders],
		adder_input_shifted[i, a] <= adder_input_value[i,a]*input_shift_values[s] + shifted_M - adder_input_shift_sel[s, i, a]*shifted_M
	)
	...
	@constraint(model, [s = 1:nof_input_sign_values, i = 1:nof_adder_inputs, a =1:nof_adders],
		adder_input_shifted[i, a]*input_sign_values[s] - sign_M + adder_input_sign_sel[s, i, a]*sign_M <= adder_inputs[i, a]
	)
	@constraint(model, [s = 1:nof_input_sign_values, i = 1:nof_adder_inputs, a =1:nof_adders],
		adder_inputs[i, a] <= adder_input_shifted[i, a]*input_sign_values[s] + sign_M - adder_input_sign_sel[s, i, a]*sign_M
	)

	# Adder output is the sum of its inputs
	@constraint(model, [a = 1:nof_adders],
		adder_outputs[a] == sum(adder_inputs[i, a] for i in 1:nof_adder_inputs)
	)
	...
	@constraint(model, [s = 0:nof_adders, i = 1:nof_adder_inputs, a = 1:nof_adders],
		adder_depth[s] - depth_M + adder_input_value_sel[s,i,a]*depth_M <= adder_input_depths[i, a] 
	)
	@constraint(model, [s = 0:nof_adders, i = 1:nof_adder_inputs, a = 1:nof_adders],
		adder_input_depths[i, a] <= adder_depth[s] + depth_M - adder_input_value_sel[s,i,a]*depth_M
	)
	...
	@constraint(model, [i = 1:nof_adder_inputs, a = 1:nof_adders], # select one input depth
		adder_input_depths[i, a] - depth_M + adder_input_max_depth_sel[i,a]*depth_M <= adder_input_max_depth[a]
	)
	@constraint(model, [i = 1:nof_adder_inputs, a = 1:nof_adders], # select one input depth
		adder_input_max_depth[a] <= adder_input_depths[i, a] + depth_M - adder_input_max_depth_sel[i,a]*depth_M
	)
	...
	@constraint(model, [a = 1:nof_adders],
		adder_depth[a] == 1 + adder_input_max_depth[a] 
	)
	
	# @objective(model, Min, sum(input_shift_values[s]*adder_input_shift_sel[s,i,a] for s in 1:nof_input_shift_values, i in 1:nof_adder_inputs, a in 1:nof_adders))
	@objective(model, Min, sum(adder_depth))
	
	optimize!(model)
end

# ╔═╡ 65a8f216-d614-4918-b104-d4fba04c6e00
model

# ╔═╡ 628d8991-6fac-4ea6-a884-0658f1b874b7
value.(adder_outputs)

# ╔═╡ 3ff682a9-89ed-4661-98a2-d1d5447e9a84
value.(adder_output_value_sel)

# ╔═╡ 7d358752-04eb-48ed-b5f3-5682a91a60e8
value.(adder_input_value)

# ╔═╡ 2be9cd5e-5d44-4e31-bd34-a900ebdf1675
value.(adder_input_shifted)

# ╔═╡ d2419c98-a58b-48b4-818d-b8908c62047c
value.(adder_inputs)

# ╔═╡ 4f9ea6df-6682-4d39-9e61-398b49c1f3c3


# ╔═╡ 7c3196f3-7e2f-4b22-8f09-4470e4fcb9ca
value.(adder_depth)
...
manifest_format = "2.0"
project_hash = "75e8f8efda0b748500af23d33749b5711df751af"
